...
//		------------------------------------------- Eliminar Pase de Bóveda en Agencia / Caja / Pases de Bóveda 2.2----------------------------------------------------
		
		//RUTA PARA GRABAR LA PANTALLA--------------------------------------------------------------------
		
		String outputFile = ""+Constantes.RUTA_CAPTURES+"\\Criticos_Agencia\\Cases_190\\190.avi";
	    ...
	    String ATM = readFile.getCellValue(filepath, "CriticosAgencia", 7, 2);
	    ...
	    doble.doubleClick(ValorParametroWF).perform();	Thread.sleep(secDelay);
		...
		OpcionCrear.click();	 Thread.sleep(secDelay);
		...
		Caja.click();	  Thread.sleep(secDelay);
			...
			try {
			    ...
			    Modalidad.click();
				WebElement DespTipoPase = ewait.until(ExpectedConditions.visibilityOfElementLocated(By.id("tipoPaseCP-trigger-picker")));
				DespTipoPase.click();	    Thread.sleep(secDelay);
		...
		WebElement titulo = ewait.until(ExpectedConditions.visibilityOfElementLocated(By.id("gridcolumn-1013")));
        Actions doble4 = new Actions(driver);
        doble4.doubleClick(titulo).perform();	
		...
		AceptarMensajeParametroWF2.click();
