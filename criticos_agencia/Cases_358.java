...
//		-------------------------------Reverso de Remesas en Agencia Emisor (estado Confirmado Ajuste Detalle) / Control de Remesas / Envíos------------------------
		...
		String outputFile = ""+Constantes.RUTA_CAPTURES+"\\Criticos_Agencia\\Cases_350\\350.avi";
		...
		String TipoUnidadEmisora = readFile.getCellValue(filepath, "CriticosAgencia", 38, 1);
		String	UnidadEmisora = readFile.getCellValue(filepath, "CriticosAgencia", 38, 2);
		String	Transporte = readFile.getCellValue(filepath, "CriticosAgencia", 38, 5);
		...
		WebElement EstadoAlterno = ewait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//li[text()='Confirmado Ajuste Detalle']")));
		EstadoAlterno.click();	
        ...
        for (WebElement option : options5) {
            if (option.getText().equals(Constantes.DENOMINACION_DIFERENCIA2)) {
                option.click();
                break;
		...
		List<WebElement> options05 = driver.findElements(By.xpath("//ul[@class='x-list-plain']/li"));
        for (WebElement option : options05) {
		...
		WebElement Cantidad2 = ewait.until(ExpectedConditions.visibilityOfElementLocated(By.id("cantidadSobrantesAjuste-inputEl")));
		Cantidad2.click();	   Thread.sleep(secDelay);
		Cantidad2.sendKeys("2");
		
		WebElement Incluir2 = ewait.until(ExpectedConditions.visibilityOfElementLocated(By.id("incluirSobrantesAjuste-btnWrap")));
		Incluir2.click();		
		
		WebElement DesplegarDenominacion2 = ewait.until(ExpectedConditions.visibilityOfElementLocated(By.id("denominacionFaltantesAjuste-inputEl")));
		...
		WebElement DesplegarClasificacion2 = ewait.until(ExpectedConditions.visibilityOfElementLocated(By.id("clasificacionFaltantesAjuste-inputEl")));
		DesplegarClasificacion2.click();
		List<WebElement> options51 = driver.findElements(By.xpath("//ul[@class='x-list-plain']/li"));
        for (WebElement option : options51) {
            if (option.getText().equals(Constantes.APTOS)) {
                option.click();
                break;
		...
		Cantidad3.sendKeys("1");
		
		WebElement Incluir3 = ewait.until(ExpectedConditions.visibilityOfElementLocated(By.id("incluirFaltantesAjuste-btnEl")));
		Incluir3.click();	  Thread.sleep(secDelay);	
		WebElement Aceptar2 = ewait.until(ExpectedConditions.visibilityOfElementLocated(By.id("aceptarAjuste-btnEl")));
		Aceptar2.click();	  Thread.sleep(secDelay);
			
		WebElement AceptarMensaje = ewait.until(ExpectedConditions.visibilityOfElementLocated(By.id("button-1031-btnInnerEl")));
