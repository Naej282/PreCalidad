...
public class Cases_345 {
	...
	public void setUp() throws Exception {
		...
		// Método para crear las carpetas dinámicamente
	    public static String createFolderStructure(String baseFolderName) {
		 // Formato para la fecha principal (yyyy-MM-dd)
        String datePattern = "yyyy-MM-dd";
		//------------------------------Formato para la subcarpeta (HH-mm-ss)--------------------------//
        String timePattern = "HH-mm-ss";
        // Obtener la fecha actual
        Date currentDate = new Date();
        SimpleDateFormat dateFormat = new SimpleDateFormat(datePattern);
        SimpleDateFormat timeFormat = new SimpleDateFormat(timePattern);
      //-----------------------Nombre de la carpeta principal (Nombre + Fecha)-----------------------//
        String mainFolderName = baseFolderName + "_" + dateFormat.format(currentDate);
        File mainFolder = new File(mainFolderName);
		        // Verificar si la carpeta principal existe
        if (!mainFolder.exists()) {
        //---------------------------rear la carpeta principal si no existe----------------------------//
            boolean created = mainFolder.mkdirs(); 
            if (created) {
                System.out.println("Carpeta creada: " + mainFolder.getAbsolutePath());
            } else {
                System.out.println("Error al crear la carpeta principal.");
            }
        } else {
            System.out.println("La carpeta ya existe: " + mainFolder.getAbsolutePath());
        }

        //---------------------Crear la subcarpeta con la hora, minuto, segundo-------------------------//
        String subFolderName = timeFormat.format(currentDate);
        File subFolder = new File(mainFolder, subFolderName);

        //--------------------------------------Crear la subcarpeta-------------------------------------//
        boolean subCreated = subFolder.mkdirs();
        if (subCreated) {
            System.out.println("Subcarpeta creada: " + subFolder.getAbsolutePath());
        } else {
...
//		------------------------------ Reverso de Remesas en Agencia Emisor (estado Confirmado Sin Ajuste) / Control de Remesas / Envíos------------------------
		...
		String outputFile = ""+Constantes.RUTA_CAPTURES+"\\Criticos_Agencia\\Cases_345\\345.avi";
		...
		String TipoUnidadEmisora = readFile.getCellValue(filepath, "CriticosAgencia", 35, 1);
		String	UnidadEmisora = readFile.getCellValue(filepath, "CriticosAgencia", 35, 2);
		String	Transporte = readFile.getCellValue(filepath, "CriticosAgencia", 35, 5);
